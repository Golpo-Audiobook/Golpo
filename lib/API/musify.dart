...
import 'package:golpo/DB/albums.db.dart';
import 'package:golpo/DB/catagory.db.dart';
import 'package:golpo/DB/mostplayed.db.dart';
import 'package:golpo/DB/playlists.db.dart';
import 'package:golpo/DB/slider.db.dart';
import 'package:golpo/DB/topten.db.dart';
import 'package:golpo/extensions/l10n.dart';
import 'package:golpo/main.dart';
import 'package:golpo/services/data_manager.dart';
import 'package:golpo/services/lyrics_manager.dart';
import 'package:golpo/services/settings_manager.dart';
import 'package:golpo/utilities/flutter_toast.dart';
import 'package:golpo/utilities/formatter.dart';
...
//
List topten = [...toptenDB];
//
List mostPlayed = [...mostplayedDB];
//
List slider = [...sliderDB];
//
List catagory = [...catagoryDB];
//
...
final userRecentlyPlayedLength = ValueNotifier<int>(userLikedPlaylists.length);
...
Future<List> getRecommendedSongs() async {
  try {
    ...
    if (globalSongs.isEmpty) {
      const playlistId = 'PLUmAU79aRKpZ1biWRCdsct-rSVnh-_VPJ';
...
Future<List> getTopTen({
  ...
  if (playlistsNum != null && query == null) {
    if (allTop.isEmpty) {
      allTop = topten.toList();
    }
    return allTop.take(playlistsNum).toList();
  }

  if (query != null && playlistsNum == null) {
    final lowercaseQuery = query.toLowerCase();
    return topten.where((playlist) {
      final lowercaseTitle = playlist['title'].toLowerCase();
      return lowercaseTitle.contains(lowercaseQuery);
    }).toList();
  }
...
Future<List> getmostPlayed({
  ...
  if (playlistsNum != null && query == null) {
    if (mostPlayedlist.isEmpty) {
      mostPlayedlist = mostPlayed.toList()..shuffle();
    }
    return mostPlayedlist.take(playlistsNum).toList();
  }

  if (query != null && playlistsNum == null) {
    final lowercaseQuery = query.toLowerCase();
    return mostPlayed.where((playlist) {
      final lowercaseTitle = playlist['title'].toLowerCase();
      return lowercaseTitle.contains(lowercaseQuery);
    }).toList();
  }
...
Future<List> getslider({
  ...
  if (playlistsNum != null && query == null) {
    if (topSlider.isEmpty) {
      topSlider = slider.toList();
    }
    return topSlider.take(playlistsNum).toList();
  }

  if (query != null && playlistsNum == null) {
    final lowercaseQuery = query.toLowerCase();
    return slider.where((playlist) {
      final lowercaseTitle = playlist['title'].toLowerCase();
      return lowercaseTitle.contains(lowercaseQuery);
    }).toList();
  }
...
////
Future<List> getAllcatagory({
  String? query,
  int? playlistsNum,
  bool onlyLiked = false,
}) async {
  if (playlistsNum != null && query == null) {
    if (getCatagory.isEmpty) {
      getCatagory = catagory.toList();
    }
    return getCatagory.take(playlistsNum).toList();
  }

  if (query != null && playlistsNum == null) {
    final lowercaseQuery = query.toLowerCase();
    return catagory.where((playlist) {
      final lowercaseTitle = playlist['title'].toLowerCase();
      return lowercaseTitle.contains(lowercaseQuery);
    }).toList();
  }
